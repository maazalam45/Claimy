...
type Props = NativeStackScreenProps<ShareStackParamList, 'Share2'>;

export function Share2Screen({ navigation }: Props) {
  return (
    <Screen padded={false} backgroundColor={colors.black}>
      <View style={[styles.root, styles.rootTheme]}>
        ...
        <View style={styles.header}>
          ...
          <Pressable onPress={() => navigation.navigate('Share3')} style={styles.postBtn}><Text style={styles.postText}>POST</Text></Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  rootTheme: { backgroundColor: '#0D2433' },
  topGlow: { ...StyleSheet.absoluteFillObject, top: -80, height: 460 },
  topGlowTheme: { backgroundColor: 'rgba(0,180,255,0.22)' },
  ...
  cardTheme: { backgroundColor: '#296E98' },
  ...
  statusOrb: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(0,214,255,0.2)', borderWidth: 1, borderColor: 'rgba(0,214,255,0.38)', alignItems: 'center', justifyContent: 'center' },
  statusOrbText: { color: '#6DEFFF', fontSize: 16, fontWeight: '800' },

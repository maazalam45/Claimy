...
type Props = NativeStackScreenProps<ShareStackParamList, 'Share1'>;

export function Share1Screen({ navigation }: Props) {
  return (
    <Screen padded={false} backgroundColor={colors.black}>
      <View style={[styles.root, styles.rootTheme]}>
        ...
        <View style={styles.header}>
          ...
          <Pressable onPress={() => navigation.navigate('Share2')} style={styles.postBtn}><Text style={styles.postText}>POST</Text></Pressable>
        </View>
        <GlassCard style={[styles.card, styles.cardTheme]}>
          <View style={styles.badges}><View style={styles.badge}><Text style={styles.badgeText}>IT'S OFFICIAL</Text></View><Text style={styles.brandMini}>CLAIMY</Text></View>
...
const styles = StyleSheet.create({
  ...
  rootTheme: { backgroundColor: '#31101F' },
  topGlow: { ...StyleSheet.absoluteFillObject, top: -80, height: 460 },
  topGlowTheme: { backgroundColor: 'rgba(255,62,130,0.24)' },
  ...
  cardTheme: { backgroundColor: '#A7355C' },
  ...
  statusOrb: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,89,162,0.24)', borderWidth: 1, borderColor: 'rgba(255,89,162,0.45)', alignItems: 'center', justifyContent: 'center' },
  statusOrbText: { color: '#FF89C6', fontSize: 16, fontWeight: '800' },

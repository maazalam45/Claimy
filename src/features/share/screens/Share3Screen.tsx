...
type Props = NativeStackScreenProps<ShareStackParamList, 'Share3'>;

export function Share3Screen({ navigation }: Props) {
  return (
    <Screen padded={false} backgroundColor={colors.black}>
      <View style={[styles.root, styles.rootTheme]}>
        ...
        <View style={styles.header}>
          ...
          <Pressable onPress={() => navigation.navigate('Share4')} style={styles.postBtn}><Text style={styles.postText}>POST</Text></Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  rootTheme: { backgroundColor: '#36140D' },
  topGlow: { ...StyleSheet.absoluteFillObject, top: -80, height: 460 },
  topGlowTheme: { backgroundColor: 'rgba(255,94,57,0.22)' },
  ...
  cardTheme: { backgroundColor: '#A44A2E' },
  ...
  statusOrb: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,71,118,0.22)', borderWidth: 1, borderColor: 'rgba(255,71,118,0.38)', alignItems: 'center', justifyContent: 'center' },
  statusOrbText: { color: '#FF739D', fontSize: 16, fontWeight: '800' },
